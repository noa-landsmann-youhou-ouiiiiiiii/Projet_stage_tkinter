...
def main():
    def reset():    #Définition de la fonction 'reset'
        try:
            ...
            pass    #J'aime pas vir des erreurs dans la console
    ...
    '''
        ...
        try:
            result = subprocess.run(['/usr/bin/bash', '-Command', ps_command], capture_output=True, text=True)
    ...
    frame_separation = Frame(root, height=3, bg=orange_altsysnet)
    frame_separation.pack(fill="x")

    identite_utilisateur_frame = Frame(root,bg="white")
    identite_utilisateur_frame.pack(side="left")

    identite_utilisateur_label = Label(identite_utilisateur_frame, text="IDENTITÉ DE L'UTILISATEUR", fg=orange_altsysnet, bg="white")
    identite_utilisateur_label.pack(pady=10)

    sous_frame_prenom = Frame(identite_utilisateur_frame, bg="white")
    sous_frame_prenom.pack(side="left")

    prenom_label = Label(sous_frame_prenom, text="Prenom",bg="white")
    prenom_label.pack()

    prenom_entry = Entry(sous_frame_prenom,bg="white",width=50)
